...
%
% Edited 30/4/2024 SLB
% Edited inputs to niftiwrite function so outputs are saved in a
% prespecified folder (assumes using with make_segmented_vol function)
...
switch lower(file_type)
    ...
    case {'nifti', 'nii'}
        ...
        if isfield(header,'original_header') % was loaded as nii
            nii = struct;
            nii.img=volume;
            nii.hdr = nifti_4dfp(header, volume, 'n');
...
%         save_nii(nii, filename);
        niftiwrite(single(nii.img), fullfile(pn, filename), nii.hdr);
