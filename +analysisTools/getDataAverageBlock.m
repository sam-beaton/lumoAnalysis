function [BA_out, BSTD_out, BT_out, blocks, blockRemoved] = getDataAverageBlock(data_in, pulse, dtPre, dtAfter, Tkeep)
...
pulse = pulse(:);
[pulse, blockRemoved] = analysisTools.chopBlocks(pulse, Nt, dtPre, dtAfter);
Nbl = length(pulse);
dt = dtPre + dtAfter;
...
for k = 1:Nbl
    ...
    baseline = mean(dataSegment(:, 1:dtPre), 2, 'omitnan');
    dataSegment = dataSegment - baseline;
