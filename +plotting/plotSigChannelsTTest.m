...
% Initialise struct-of-arrays
customPairsExthab.Src   = [];
customPairsExthab.Det   = [];
customPairsExthab.Color = {};
customPairsExthab.Style = {};
customPairsExthab.Width = [];  % Optional: thicker lines for both chroms

% Build channel ID strings
exthabHbO_IDs = strcat(string(extHabHbOTable.Source), '_', string(extHabHbOTable.Detector));
exthabHbR_IDs = strcat(string(extHabHbRTable.Source), '_', string(extHabHbRTable.Detector));
exthabAll_IDs = union(exthabHbO_IDs, exthabHbR_IDs);
overlapExthab = intersect(exthabHbO_IDs, exthabHbR_IDs);  % HbO + HbR in exthab

% Activation channel IDs for line style reference
actHbO_IDs = strcat(string(act12HbOTable.Source), '_', string(act12HbOTable.Detector));
actHbR_IDs = strcat(string(act12HbRTable.Source), '_', string(act12HbRTable.Detector));
actBoth_IDs = intersect(actHbO_IDs, actHbR_IDs);

% Loop through all unique exthab channels
for i = 1:numel(exthabAll_IDs)
    pairID = exthabAll_IDs(i);
    splitID = split(pairID, '_');
    src = str2double(splitID{1});
    det = str2double(splitID{2});

    % Determine colour
    if ismember(pairID, overlapExthab)
        colour = colours.Both;  % Both chromophores → green
    elseif ismember(pairID, exthabHbO_IDs)
        colour = colours.HbO;   % HbO only → red
    else
        colour = colours.HbR;   % HbR only → blue
    end

    % Determine style
    if ismember(pairID, actBoth_IDs)
        style = '-';   % Solid
        width = 2;     % Thicker for emphasis
    elseif ismember(pairID, actHbO_IDs)
        style = '--';  % Dashed
        width = 1;
    elseif ismember(pairID, actHbR_IDs)
        style = '-.';   % Dotted
        width = 1;
    else
        style = ':';  % Fallback: dash-dot for unmatched (optional)
        width = 0.5;
    end

    % Append to struct
    customPairsExthab.Src(end+1)   = src;
    customPairsExthab.Det(end+1)   = det;
    customPairsExthab.Color{end+1} = colour;
    customPairsExthab.Style{end+1} = style;
    customPairsExthab.Width(end+1) = width;
end
...
% Plot 12mo exthab
testFuncs.testPlotCap(data.info, paramsPlot, customPairsExthab);
